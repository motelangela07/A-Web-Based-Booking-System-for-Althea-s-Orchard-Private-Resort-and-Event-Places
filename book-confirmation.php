...
<link rel="stylesheet" type="text/css" href="css/book-confirmation.css">
...
<?php
    $ref_no = $_SESSION['ref_no'];
        $query = $conn->query("SELECT * FROM book_info WHERE ref_no = '$ref_no'");
		$fetch = $query->fetch_array();    
        
        $query1 = $conn->query("SELECT * FROM book_details WHERE booking_ref_no = '$ref_no'");
		$fetch1 = $query1->fetch_array(); 
       
        $query11 = $conn->query("SELECT * FROM book_details WHERE booking_ref_no = '$ref_no'");
		$fetch11 = $query11->fetch_array(); 
      
        //Pax Identificator kung ilang yung add ons na pax nya 
        $pax_num = $fetch1['additional_package'];
        $pax_total = $pax_num * 500; 
        $query = $conn->query("SELECT * FROM book_details WHERE booking_ref_no = '$ref_no'");
        $fetch = $query->fetch_array();  

        $query2 = $conn->query("SELECT * FROM book_info WHERE ref_no = '$ref_no'");
        $fetch2 = $query2->fetch_array();  
        $query3 = $conn->query("SELECT * FROM book_info WHERE ref_no = '$ref_no'");
        $fetch3 = $query3->fetch_array(); 
        $query4 = $conn->query("SELECT * FROM book_info WHERE ref_no = '$ref_no'");
        $fetch4 = $query4->fetch_array(); 

        $booking_start  = $fetch['booking_start']; 
        $booking_end    = $fetch['booking_end'];   
        ...
        <div class="row justify-content-start">
            <div class="col-12">
                <div id="bg1">
                    ...
                    <h4>Staycation Itinerary</h4>
                    <b>Staycation Reference No. : </b>
                    <?php echo $ref_no;?> <br>
                    <b>Accommodation Type :</b> 
                    <?php echo $fetch1['regular_accommodation'];?> <br>
                    <b>Date Range :</b> 
                    <?php echo $fetch1['date_range'];?> <br>
                    <b>Additional Guests :</b> 
                    <?php echo $pax_num ;?> (500 per pax) <br>
                    <b>Total Fee :  </b> ₱
                    <?php echo $fetch1['total'];?><br>
                    <b>Full Name : </b>
                    <?php echo $fetch2['book_fname'];  ?> 
                    <?php echo $fetch2['book_lname'];  ?> <br>
                    <b>Contact : </b>
                    <?php echo $fetch2['book_contact'];  ?> <br>
                    <b>Address : </b>
                    <?php echo $fetch2['book_address'];  ?> <br>
                    <b>Email : </b>
                    <?php echo $fetch2['book_email'];  ?> <br>
                    <?php
                   ?>
</div>
...
<script>
    function printSummary() {
        ...
        printWindow.document.write('<html><head><title>Staycation Booking Itinerary</title></head><body>');
...
function calculateTotal() {
    ...
    document.getElementById('total').innerText = 'Total Fee: ₱' + totalFee;
}
