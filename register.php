...
>
  ...
  <input type="text" pattern=".{11}" name="con"class="form-control" maxlength="11" title="Must contain 11 numbers only" required>
...
if (isset($_POST['sent'])){
 ...
 $email =   $_POST['email'];
 $pass =   $_POST['pass'];
$query =$conn->query("SELECT * FROM `user_account` WHERE `user_name`='$uname'");
...
if($count_uname>=1){
    echo '<script>function myFunction() {
        ...
        $query1 =$conn->query("SELECT * FROM `user_account` WHERE `user_email`='$email'");
        ...
        $query2 =$conn->query("SELECT * FROM `user_account` WHERE `user_contact`='$con'");
        $count_con = mysqli_num_rows($query2);
        if($count_con>=1){
            echo '<script>function myFunction() {
                swal({
                title: "Failed!",
                text: "Contact is Already Exist.",
                  icon: "error",
                type: "error"
                }).then(function() {
                window.location = "index.php";
            ...
            $sql_user="INSERT INTO `user_account` VALUES(null,'$uname','$add','$con','$email','$pass')";
