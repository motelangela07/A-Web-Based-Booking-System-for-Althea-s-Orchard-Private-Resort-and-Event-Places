...
<div class="card-body p-0 booking-table">
    <div class="table-responsive booking-table-responsive">
                <?php
                $app_id = $_GET['app_id'];
                $q_e = $conn->query("SELECT * FROM `photo_details` WHERE `photo_id`='$app_id'");
    		    $f_e = $q_e->fetch_array();
                ?>
    ...
    <div class="row">
        <div class="col-md-6">
            <div class="form-group">
                <label for="photo_ref_no">Booking Reference Number:</label>
                <b><?php echo $f_e['photo_ref_no'] ?><b>
            </div>
            <div class="form-group">
                <label for="photo_selected_services">Booking Services:</label>
                <b><?php echo $f_e['photo_selected_services'] ?></b>
            </div>
            <!-- Add more fields as needed -->
        </div>
        <div class="col-md-6">
            <div class="form-group">
                <label for="photo_date">Booking Date:</label>
                <input type="text" class="form-control" value="<?php echo $f_e['photo_date'] ?>" readonly>
                <input type="hidden" class="form-control" value="<?php echo $f_e['photo_id'] ?>" name="photo_id">
            </div>
            <!-- Add more fields as needed -->
        </div>
    </div>
    ...
    <div class="form-group">
        ...
        <a href="photoshoot.php" class="btn btn-danger">Cancel</a>
    </div>
</form>
                          <?php   
                    ...
                    if(isset($_POST['save_update'])){
                      ...
                      $update = $conn->query("UPDATE `photo_details` SET `status`= '$status' WHERE `photo_id`= '$photo_id'");
                      if ($update) {
                        echo '<script>
                            swal({
                                title: "Success!",
                        ...
                        echo '<script>
                            swal({
                                ...
                                window.location = "photoshoot-update.php?app_id=' . $photo_id . '";
                            });
                        </script>';
                    }
