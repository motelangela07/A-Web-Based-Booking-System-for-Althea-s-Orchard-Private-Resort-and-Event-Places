...
<div class="card-body p-0 booking-table">
    <div class="table-responsive booking-table-responsive">
                <?php
                $app_id = $_GET['app_id'];
                $q_e = $conn->query("SELECT * FROM `book_details` WHERE `booking_id`='$app_id'");
    		    $f_e = $q_e->fetch_array();
                ?>
    ...
    <div class="row">
        <div class="col-md-6">
            <div class="form-group">
                <label for="booking_ref_no">Booking Reference Number:</label>
                <b><?php echo $f_e['booking_ref_no'] ?><b>
            </div>
            <div class="form-group">
                <label for="booking_services">Booking Services:</label>
                <b><?php echo $f_e['booking_services'] ?></b>
            </div>
        ...
        <div class="col-md-6">
            <div class="form-group">
                ...
                <input type="text" class="form-control" value="<?php echo $f_e['booking_start'] ?>" readonly>
                <input type="hidden" class="form-control" value="<?php echo $f_e['booking_id'] ?>" name="booking_id">
            </div>
            <!-- Add more fields as needed -->
        </div>
    </div>
    ...
    <div class="form-group">
        ...
        <a href="staycation.php" class="btn btn-danger">Cancel</a>
    </div>
</form>
                          <?php   
                    ...
                    if(isset($_POST['save_update'])){
                      ...
                      $update = $conn->query("UPDATE `book_details` SET `status`= '$status' WHERE `booking_id`= '$booking_id'");
                      if ($update) {
                        echo '<script>
                            swal({
                                title: "Success!",
                        ...
                        echo '<script>
                            swal({
                                ...
                                window.location = "staycation-update.php?app_id=' . $booking_id . '";
                            });
                        </script>';
                    }
